...
install.packages(c("schrute", "tidymodels"))
...
office_lines <- theoffice %>% 
  ...
  mutate(
    nlines = n(),
    jim_lines = sum(character == "Jim") / nlines, 
    pam_lines = sum(character == "Pam") / nlines,
    michael_lines = sum(character == "Michael") / nlines,
    dwight_lines = sum(character == "Dwight") / nlines,
  ) %>%
  ungroup() %>% 
  select(season, episode, episode_name, contains("_lines")) %>% 

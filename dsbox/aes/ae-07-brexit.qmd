...
brexit %>% 
  ...
  ggplot(aes(y = opinion, fill = opinion)) +
  geom_col(aes(x = proportion)) +
  facet_wrap(~region, nrow = 1, labeller = label_wrap_gen(width = 12)) +
  guides(fill="none") +
  scale_x_continuous(breaks = breaks_width(0.1), labels = label_number()) +
  labs(
    title = "Was Britain right/wrong to vote to leave EU?",
    subtitle = "YouGov Survey Results, 2-3 September 2019",
    caption = "Source: bit.ly/2lCJZVg",
...
brexit %>% 
  count(opinion, region) %>% 
  mutate(proportion = n / sum(n))  %>% 
  ggplot(aes(y = opinion, fill = region)) +
  geom_col(aes(proportion), position = "dodge") 
```
